...
whose dilution term is the only one that scales with M.
...
def main(simulate=False):
    ...
    M_100x = (STD_NS_EVALS / 100 - fixed) / (n_pools * per_iteration)
    ...
    axb.annotate(rf"$\mathscr{{M}}\to 0$: {ceiling:.0f}$\times$",
                 ...
                 fontsize=ps.ANNOTATION_FONTSIZE)
    for M, txt in ((M_100x, rf"$\mathscr{{M}}={M_100x:.0f}$"), (M_RUN, rf"$\mathscr{{M}}={M_RUN}$")):
        axb.plot([M], [reduction(M)], "o", color="k", ms=ps.MS)
        axb.annotate(txt, xy=(M, reduction(M)), xytext=(M + 10, reduction(M) + 6),
                     fontsize=ps.ANNOTATION_FONTSIZE)
    ...
    # binom needs an integer number of trials, so quote the coverage at the nearest step
    print(f"M for exactly 100x: {M_100x:.1f}  -> P(all refreshed) = {p_all_touched(round(M_100x))[0]:.2e}, "
          f"E[untouched] = {K*(1-p)**M_100x:.0f}")
    for M in (47, 87, 100, 132, 159, 183):
