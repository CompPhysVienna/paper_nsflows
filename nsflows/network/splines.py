...
from nsflows.tools.util import searchsorted, torch_ns_sqrt, torch_ns_cbrt
...
def cubic_B_spline(  # nonuniform , # w/o identity
    ...
    if inverse:
        ...
        p_[one_root_mask] = torch_ns_cbrt((-depressed_1[one_root_mask] + torch_ns_sqrt(-discriminant[one_root_mask])) / 2.)

        p = p_[one_root_mask]
        q = torch_ns_cbrt((-depressed_1[one_root_mask] - torch_ns_sqrt(-discriminant[one_root_mask])) / 2.)
        ...
        theta = torch.atan2(torch_ns_sqrt(discriminant[three_roots_mask]), -depressed_1[three_roots_mask])
        theta /= 3.
        ...
        root_scale = 2 * torch_ns_sqrt(-depressed_2[three_roots_mask])
        ...
        alpha = (-b + torch_ns_sqrt(b.pow(2) - 4 * a * c)) / (2 * a)
